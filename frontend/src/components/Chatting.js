...
const Chatting = ({ roomNumber, lastMessage }) => {
			...
			>
				{messages.map((message, index) => (
					<Box
						key={index}
						sx={{
							textAlign: message.nickname === `${user.nickname}`
								? 'right'
								: 'left',
							margin: '10px 0',
						}}
					>
						<Typography
							...
							sx={{
								...
								padding: '8px 16px',
								borderRadius: '20px',
								backgroundColor: message.nickname === `${user.nickname}`
									? '#0B93F6'
									: '#e0e0e0',
								color: message.nickname === `${user.nickname}`
									? '#fff'
									: '#000',
							}}
						>
